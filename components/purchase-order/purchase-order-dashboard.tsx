...
const PurchaseOrderDashboard = ({companies, stats, plData} : any) => {
    ...
    const resetFilters = () => setFilters({ company: "all", startDate: undefined, endDate: undefined, month: "all", year: currentYear.toString() });
    ...
    const fetchRevenueDetails = async () => {
        ...
        if (filters.company !== "all") {
            const companyName = companies.find((c: any) => String(c.id) === filters.company)?.name;
    ...
    const filteredStats = revenueDetails.reduce((acc, item) => {
    ...
    const collectionEfficiency = filteredStats.totalBilledAmount
        ? Math.round((filteredStats.totalCollectedAmount / filteredStats.totalBilledAmount) * 100)
        : 0;
    ...
    return (
        <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <Popover>
                            ...
                            <PopoverContent className="w-auto p-0 rounded-xl">
                                <Calendar mode="single" selected={filters.startDate} onSelect={date => updateFilter("startDate", date)} />
                            </PopoverContent>
                        </Popover>
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <Popover>
                            ...
                            <PopoverContent className="w-auto p-0 rounded-xl">
                                <Calendar mode="single" selected={filters.endDate} onSelect={date => updateFilter("endDate", date)} />
                            </PopoverContent>
                        </Popover>
                    </div>
                    ...
                    <div className="space-y-2">
                        <Label>Month</Label>
                        <Select value={filters.month} onValueChange={val => updateFilter("month", val)}>
                            <SelectTrigger className="w-full rounded-xl">
                                <SelectValue placeholder="Select Month" />
                            </SelectTrigger>
                            <SelectContent>
                                <SelectItem value="all">All Months</SelectItem>
                                {allMonths.map((month, i) => <SelectItem key={i} value={i.toString()}>{month}</SelectItem>)}
                            </SelectContent>
                        </Select>
                    </div>
                    ...
                    <div className="space-y-2">
                        <Label>Year</Label>
                        <Select value={filters.year} onValueChange={val => updateFilter("year", val)}>
                            <SelectTrigger className="w-full rounded-xl">
                    ...
                    <div className="flex items-end">
                        <Button variant="destructive" className="w-full rounded-xl shadow-sm" onClick={resetFilters}>
                ...
                <Card>
                    <CardContent>
                        <div className="flex justify-between">
                            ...
                            <span className="text-blue-500 font-bold">₹{filteredStats.totalBilledAmount.toLocaleString()}</span>
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card>
                    <CardContent>
                        <div className="flex justify-between">
                            ...
                            <span className="text-blue-500 font-bold">₹{filteredStats.totalCollectedAmount.toLocaleString()}</span>
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card>
                    <CardContent>
                        <div className="flex justify-between">
                            ...
                            <span className="text-blue-500 font-bold">₹{filteredStats.totalOverdueAmount.toLocaleString()}</span>
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card>
                    <CardContent>
                        <div className="flex justify-between">
                            ...
                            <span className="text-blue-500 font-bold">{collectionEfficiency}%</span>
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card>
                    <CardContent>
                        <div className="flex justify-between">
                            ...
                            <span className="text-blue-500 font-bold">{revenueDetails.length}</span>
                        </div>
                    </CardContent>
                </Card>
            </div>
            ...
            {/* REVENUE TABLE */}
            <Card>
                <CardHeader><h2 className="text-xl font-bold">Revenue Details</h2></CardHeader>
                <CardContent>
                    <Table>
                        <TableHeader>
                            <TableRow>
                        ...
                        <TableBody>
                            {revenueDetails.map((item, i) => {
                                const pendingAmount = Math.max((item.amount || 0) - (item.collectedAmount || 0), 0);
                                return (
                                    <TableRow key={i}>
                                        <TableCell>{item.companyName || "-"}</TableCell>
                                        <TableCell>{item.poNumber || "-"}</TableCell>
                                        <TableCell>₹{Math.round(item.amount || 0).toLocaleString()}</TableCell>
                                        <TableCell>₹{Math.round(item.collectedAmount || 0).toLocaleString()}</TableCell>
                                        <TableCell>₹{Math.round(pendingAmount).toLocaleString()}</TableCell>

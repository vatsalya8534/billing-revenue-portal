...
export const projectMonthlyPLSchema = z.object({
  ...
  fms: z.number(),
  spare: z.number(),
  billableAmount: z.number().default(0),
  resourceUsed: z.number().default(0),
  otherCost: z.any(),
  createdAt: z.date().nullable().optional(),
  updatedAt: z.date().optional(),
});
...
export const projectSchema = z.object({
  ...
  endDate: z.union([z.date(), z.string()]).nullable().optional(),
  poValue: z.number().min(1, "PO value is required"),
  resourceCount: z.number().min(1, "total resource count"),
  billingPlanId: z.string().min(1, "Billing Plan is required"),
  orderType: z.enum(Object.values(OrderType)),
  projectedProfit: z.number().default(0),
  status: z.enum(Object.values(Status)),
